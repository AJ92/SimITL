...
// How many bytes can we write *this* iteration without overflowing transmit
// buffers or overstressing the OpenLog?
...
// Print the null-terminated string 's' to the blackbox device and return the
...
int blackboxWriteString(const char *s) {
    ...
    switch (blackboxConfig()->device) {
        ...
        case BLACKBOX_DEVICE_SDCARD:
            ...
            // length = strlen(s);
            // afatfs_fwrite(blackboxSDCard.logFile, (const uint8_t*) s,
            // length); // Ignore failures due to buffers filling up
            break;
#endif  // USE_SDCARD
...
/**
 * If there is data waiting to be written to the blackbox device, attempt to
 * write (a portion of) that now.
 *
 * Intended to be called regularly for the blackbox device to perform
 * housekeeping.
 */
...
#ifdef USE_FLASHFS
            /*
             * This is our only output device which requires us to call flush()
             * in order for it to write anything. The other devices will
             * progressively write in the background without Blackbox calling
             * anything.
             */
        case BLACKBOX_DEVICE_FLASH:
...
#ifdef USE_SDCARD
        case BLACKBOX_DEVICE_SDCARD:
            /* SD card will flush itself without us calling it, but we need to
             * call flush manually in order to check if it's done yet or not!
             */
            // TODO: needed?
            return true;
#endif  // USE_SDCARD
...
bool blackboxDeviceOpen(void) {
    switch (blackboxConfig()->device) {
        case BLACKBOX_DEVICE_SERIAL:
            blackboxFile = fopen("blackbox.bbl", "wb");
...
#ifdef USE_FLASHFS
        case BLACKBOX_DEVICE_FLASH:
            // Some flash device, e.g., NAND devices, require explicit close to
...
static void blackboxLogDirCreated(afatfsFilePtr_t directory) {
    ...
    &blackboxSDCard.logDirectoryFinder);
...
static bool blackboxSDCardBeginLog(void) {
    ...
    case BLACKBOX_SDCARD_ENUMERATE_FILES:
        while (afatfs_findNext(blackboxSDCard.logDirectory,
    &blackboxSDCard.logDirectoryFinder, &directoryEntry) ==
    AFATFS_OPERATION_SUCCESS) { if (directoryEntry &&
    !fat_isDirectoryEntryTerminator(directoryEntry)) {
                // If this is a log file, parse the log number from the filename
                if (strncmp(directoryEntry->filename, LOGFILE_PREFIX,
    strlen(LOGFILE_PREFIX)) == 0
                    && strncmp(directoryEntry->filename + 8, LOGFILE_SUFFIX,
    strlen(LOGFILE_SUFFIX)) == 0) { char logSequenceNumberString[6];

                    memcpy(logSequenceNumberString, directoryEntry->filename +
    3, 5); logSequenceNumberString[5] = '\0';

                    blackboxSDCard.largestLogFileNumber = MAX((uint32_t)
    atoi(logSequenceNumberString), blackboxSDCard.largestLogFileNumber);
                }
            } else {
                // We're done checking all the files on the card, now we can
    create a new log file afatfs_findLast(blackboxSDCard.logDirectory);

                blackboxSDCard.state =
    BLACKBOX_SDCARD_CHANGE_INTO_LOG_DIRECTORY; goto doMore;
            }
        }
        break;
...
/**
 * Begin a new log (for devices which support separations between the logs of
 * multiple flights).
 *
 * Keep calling until the function returns true (open is complete).
 */
...
/**
 * Terminate the current log (for devices which support separations between the
 * logs of multiple flights).
 *
 * retainLog - Pass true if the log should be kept, or false if the log should
 * be discarded (if supported).
 *
    ...
    switch (blackboxConfig()->device) {
        ...
        case BLACKBOX_DEVICE_SDCARD:
            ...
            if (
                ...
                // Don't bother waiting the for the close to complete, it's
            queued now and will complete eventually blackboxSDCard.logFile =
            NULL; blackboxSDCard.state = BLACKBOX_SDCARD_READY_TO_CREATE_LOG;
                return true;
            }
            */
            return true;
...
#ifdef USE_SDCARD
        case BLACKBOX_DEVICE_SDCARD:
            return true;  // sdcard_isInserted() && sdcard_isFunctional() &&
...
/**
 * Call once every loop iteration in order to maintain the global
 * blackboxHeaderBudget with the number of bytes we can transmit this iteration.
 */
void blackboxReplenishHeaderBudget(void) {
    ...
    blackboxHeaderBudget =
        MIN(MIN(freeSpace,
                blackboxHeaderBudget + blackboxMaxHeaderBytesPerIteration),
            BLACKBOX_MAX_ACCUMULATED_HEADER_BUDGET);
}

/**
 * You must call this function before attempting to write Blackbox header bytes
 * to ensure that the write will not cause buffers to overflow. The number of
 * bytes you can write is capped by the blackboxHeaderBudget. Calling this
 * reservation function doesn't decrease blackboxHeaderBudget, so you must
 * manually decrement that variable by the number of bytes you actually wrote.
 *
 * When the Blackbox device is FlashFS, a successful return code guarantees that
 * no data will be lost if you write that many bytes to the device (i.e.
 * FlashFS's buffers won't overflow).
 *
 * When the device is a serial port, a successful return code guarantees that
 * Cleanflight's serial Tx buffer will not overflow, and the outgoing bandwidth
 * is likely to be small enough to give the OpenLog time to absorb MicroSD card
 * latency. However the OpenLog could still end up silently dropping data.
 *
 * Returns:
 *  BLACKBOX_RESERVE_SUCCESS - Upon success
 *  BLACKBOX_RESERVE_TEMPORARY_FAILURE - The buffer is currently too full to
 * service the request, try again later BLACKBOX_RESERVE_PERMANENT_FAILURE - The
 * buffer is too small to ever service this request
 */
    ...
    switch (blackboxConfig()->device) {
        ...
        case BLACKBOX_DEVICE_FLASH:
            ...
            if (bytes > (int32_t)flashfsGetWriteBufferFreeSpace()) {
                /*
                 * The write doesn't currently fit in the buffer, so try to make
                 * room for it. Our flushing here means that the Blackbox header
                 * writing code doesn't have to guess about the best time to ask
                 * flashfs to flush, and doesn't stall waiting for a flush that
                 * would otherwise not automatically be called.
                 */
